...
const collection = async (): Promise<Collection<GameMapPlanDoc>> => {
  if (!mongoUri) {
  ...
  return globalThis.foundryPlannerMongoClient.db(mongoDbName).collection<GameMapPlanDoc>("game_map_plans");
};
...
export const proxyToBackend = async (request: NextRequest, path = "") => {
  ...
  const target = `${backendBase}/api/game-map-planner${path}${sourceUrl.search}`;
...
export const createPlan = async (request: NextRequest) => {
  try {
    ...
    const userId = request.headers.get("x-user-id");
    if (!userId) {
      return NextResponse.json({ error: "Sign in to share game map plans" }, { status: 401 });
    }
...
export const updatePlan = async (request: NextRequest, id: string) => {
  try {
    ...
    const userId = request.headers.get("x-user-id");
    if (!userId) {
      return NextResponse.json({ error: "Sign in to update game map plans" }, { status: 401 });
    }
    ...
    const body = await request.json();
    const updates: Partial<GameMapPlanDoc> = { updatedAt: new Date() };
...
export const deletePlan = async (request: NextRequest, id: string) => {
  try {
    ...
    const userId = request.headers.get("x-user-id");
    if (!userId) {
      return NextResponse.json({ error: "Sign in to delete game map plans" }, { status: 401 });
    }
