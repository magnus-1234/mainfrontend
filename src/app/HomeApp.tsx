...
const SvsAppointmentPlanner = dynamic(() => import("./svs-appointment-planner/SvsAppointmentPlanner"), { ssr: false });
import "./svs-appointment-planner/svs-appointment-planner.css";
const WarAcademyCalculator = dynamic(() => import("./war-academy-calculator/WarAcademyCalculator"), { ssr: false });
...
type FireCrystalStep = {
  id: string;
  label: string;
  tier: string;
  fireCrystals: number;
  refinedFireCrystals: number;
  meat: number;
  wood: number;
  coal: number;
  iron: number;
...
const fireCrystalStepRows = [
  { prefix: "30", count: 4, final: "FC1", tier: "FC1", fc: 132, rfc: 0, meat: 67, wood: 67, coal: 13, iron: 3.3, days: 7, power: 57000, prereq: "Furnace 30 + Research Center 30" },
  { prefix: "FC1", count: 4, final: "FC2", tier: "FC2", fc: 158, rfc: 0, meat: 72, wood: 72, coal: 14, iron: 3.6, days: 9, power: 57000, prereq: "Embassy FC1 + Lancer Camp FC1" },
  { prefix: "FC2", count: 4, final: "FC3", tier: "FC3", fc: 238, rfc: 0, meat: 79, wood: 79, coal: 15, iron: 3.9, days: 11, power: 57000, prereq: "Embassy FC2 + Infantry Camp FC2" },
  { prefix: "FC3", count: 4, final: "FC4", tier: "FC4", fc: 280, rfc: 0, meat: 82, wood: 82, coal: 16, iron: 4.1, days: 12, power: 63000, prereq: "Embassy FC3 + Marksman Camp FC3" },
  { prefix: "FC4", count: 4, final: "FC5", tier: "FC5", fc: 335, rfc: 0, meat: 84, wood: 84, coal: 16, iron: 4.2, days: 14, power: 63000, prereq: "Embassy FC4 + Lancer Camp FC4" },
  { prefix: "FC5", count: 4, final: "FC6", tier: "FC6", fc: 200, finalFc: 100, rfc: 10, finalRfc: 20, meat: 96, wood: 96, coal: 19, iron: 4.8, days: 15, power: 67000, prereq: "Embassy FC5 + Infantry Camp FC5" },
  { prefix: "FC6", count: 4, final: "FC7", tier: "FC7", fc: 240, finalFc: 120, rfc: 15, finalRfc: 30, meat: 100, wood: 100, coal: 21, iron: 5.4, days: 18, power: 67000, prereq: "Embassy FC6 + Marksman Camp FC6" },
  { prefix: "FC7", count: 4, final: "FC8", tier: "FC8", fc: 240, finalFc: 120, rfc: 20, finalRfc: 40, meat: 130, wood: 130, coal: 26, iron: 6.6, days: 20, power: 67000, prereq: "Embassy FC7 + Lancer Camp FC7" },
  { prefix: "FC8", count: 4, final: "FC9", tier: "FC9", fc: 280, finalFc: 140, rfc: 30, finalRfc: 60, meat: 140, wood: 140, coal: 29, iron: 7.2, days: 13, power: 72000, prereq: "Embassy FC8 + Infantry Camp FC8" },
  { prefix: "FC9", count: 4, final: "FC10", tier: "FC10", fc: 350, finalFc: 175, rfc: 70, finalRfc: 140, meat: 160, wood: 160, coal: 33, iron: 8.4, days: 20, power: 72000, prereq: "Embassy FC9 + Marksman Camp FC9" },
];

const fireCrystalSteps: FireCrystalStep[] = fireCrystalStepRows.flatMap((row) => {
  const subSteps = Array.from({ length: row.count }, (_, index) => ({
    id: `${row.prefix}-${index + 1}`,
    label: `${row.prefix}-${index + 1}`,
    tier: row.tier,
    fireCrystals: row.fc,
    refinedFireCrystals: row.rfc,
    meat: row.meat,
    wood: row.wood,
    coal: row.coal,
    iron: row.iron,
    days: row.days,
    power: row.power,
    prerequisite: index === 0 ? row.prereq : "-",
  }));
  ...
  return [
    ...subSteps,
    {
      ...
      label: row.final,
      tier: row.tier,
      fireCrystals: row.finalFc ?? row.fc,
      refinedFireCrystals: row.finalRfc ?? row.rfc,
      meat: row.meat,
      wood: row.wood,
      coal: row.coal,
      iron: row.iron,
...
const fireCrystalStartOptions = [{ id: "30", label: "Level 30" }, ...fireCrystalSteps.map((step) => ({ id: step.id, label: step.label }))];
const fireCrystalEmptyStock: FireCrystalStock = { fireCrystals: 0, refinedFireCrystals: 0, meat: 0, wood: 0, coal: 0, iron: 0 };
const fireCrystalStockFields: { key: keyof FireCrystalStock; label: string; suffix?: string; icon: string }[] = [
  { key: "fireCrystals", label: "Fire Crystals", icon: "/svs-resources/fire-crystal.png" },
  { key: "refinedFireCrystals", label: "Refined FC", icon: "/svs-resources/fire-crystal-shard.png" },
  { key: "meat", label: "Meat", suffix: "M", icon: "/home-icons/gift-code.svg" },
  { key: "wood", label: "Wood", suffix: "M", icon: "/wiki/buildings/furnace/1d5f9abc1441.png" },
  { key: "coal", label: "Coal", suffix: "M", icon: "/wiki/buildings/coal-mine/9613a7ea6f3b.png" },
  { key: "iron", label: "Iron", suffix: "M", icon: "/wiki/buildings/iron-mine/a1184f935155.png" },
];

const addFireCrystalCost = (total: FireCrystalStock, step: FireCrystalStep): FireCrystalStock => ({
  fireCrystals: total.fireCrystals + step.fireCrystals,
  refinedFireCrystals: total.refinedFireCrystals + step.refinedFireCrystals,
  meat: total.meat + step.meat,
  wood: total.wood + step.wood,
  coal: total.coal + step.coal,
  iron: total.iron + step.iron,
});

const fireCrystalOptionIndex = (id: string) => (id === "30" ? -1 : fireCrystalSteps.findIndex((step) => step.id === id));

const fireCrystalCostFor = (fromId: string, toId: string) => {
  const fromIndex = fireCrystalOptionIndex(fromId);
  const toIndex = fireCrystalOptionIndex(toId);
  const steps = toIndex > fromIndex ? fireCrystalSteps.slice(fromIndex + 1, toIndex + 1) : [];
  const total = steps.reduce(addFireCrystalCost, fireCrystalEmptyStock);
  return {
    steps,
    total,
    days: steps.reduce((sum, step) => sum + step.days, 0),
    power: steps.reduce((sum, step) => sum + step.power, 0),
  };
};

const fireCrystalTierSummary = fireCrystalStepRows.map((row) => {
  const steps = fireCrystalSteps.filter((step) => step.tier === row.tier);
  const total = steps.reduce(addFireCrystalCost, fireCrystalEmptyStock);
  return { tier: row.tier, prerequisite: row.prereq, total, days: steps.reduce((sum, step) => sum + step.days, 0) };
});
...
Keep farming clean.`,
  ...
  ...(chatArtTemplates as MessageTemplate[]),
...
const calculatorMenuItems: { label: string; icon: string; menu: "chiefGear" | "chiefCharm" | "vip" | "fireCrystals" | "warAcademy"; href: string }[] = [
  { label: "VIP", icon: "star", menu: "vip", href: "/vip-calculator" },
  { label: "Fire Crystals", icon: "flame", menu: "fireCrystals", href: "/fire-crystals" },
  ...
  { label: "War Academy", icon: "book", menu: "warAcademy", href: "/war-academy-calculator" },
];
...
const wosWikiMenuItems: { label: string; icon: string; menu?: ActiveMenu; href?: string; status?: string; disabled?: boolean }[] = [
  ...
  { label: "Posters", icon: "image", menu: "wikiPosters", href: "/wiki/posters" },
...
const scrapedWosPostersData = wikiPostersData as unknown as WosWikiPostersData;
const scrapedWosPosters = scrapedWosPostersData.posters;
const posterCategories = scrapedWosPostersData.categories.map((category) => category.label);
...
const sidebarWikiItems: { label: string; mobileLabel: string; icon: string; menu: "wikiBuildings" | "wikiHeroes" | "wikiPosters"; href: string }[] = [
  ...
  { label: "Posters", mobileLabel: "Posters", icon: "image", menu: "wikiPosters", href: "/wiki/posters" },
];

const sidebarCalculatorItems: { label: string; mobileLabel: string; icon: string; menu: "chiefGear" | "chiefCharm" | "vip" | "fireCrystals" | "warAcademy"; href: string }[] = [
  { label: "VIP", mobileLabel: "VIP", icon: "star", menu: "vip", href: "/vip-calculator" },
  { label: "Fire Crystals", mobileLabel: "FC", icon: "flame", menu: "fireCrystals", href: "/fire-crystals" },
  ...
  { label: "War Academy", mobileLabel: "War", icon: "book", menu: "warAcademy", href: "/war-academy-calculator" },
];
...
const navVisuals: Partial<Record<ActiveMenu, NavVisual>> = {
  ...
  fireCrystals: { src: "/svs-resources/fire-crystal.png", alt: "Fire crystal" },
  chiefGear: { src: "/woscalc/gear/helmet-gold.png", alt: "Chief gear" },
  chiefCharm: { src: "/woscalc/charms/Infantry_16.png", alt: "Chief charm" },
  warAcademy: { src: "/svs-resources/fire-crystal-shard.png", alt: "War Academy shard" },
...
const landingToolVisuals: Partial<Record<ActiveMenu, { src: string; alt: string; tone: "gold" | "ice" | "ember" | "blue" | "green" | "violet" }>> = {
  ...
  warAcademy: { src: "/svs-resources/fire-crystal-shard.png", alt: "Whiteout Survival War Academy shard", tone: "ember" },
  ...
  wikiPosters: { src: "/wiki/posters/heroes/001-9d69714303ab.jpg", alt: "Whiteout Survival official poster", tone: "blue" },
...
const hashMenuAliases: Record<string, ActiveMenu> = {
  ...
  "#fire-crystal-calculator": "fireCrystals",
  "#fc-calculator": "fireCrystals",
  ...
  "#war-academy-calculator": "warAcademy",
  "#war-academy": "warAcademy",
  "#t11-calculator": "warAcademy",
...
function BrandLogo({ brand }: { brand: ShareBrand }) {
  const svgs: Record<ShareBrand, ReactNode> = {
    whatsapp: <svg fill="currentColor" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>WhatsApp</title><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z"/></svg>,
    discord: <svg fill="currentColor" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>Discord</title><path d="M20.317 4.3698a19.7913 19.7913 0 00-4.8851-1.5152.0741.0741 0 00-.0785.0371c-.211.3753-.4447.8648-.6083 1.2495-1.8447-.2762-3.68-.2762-5.4868 0-.1636-.3933-.4058-.8742-.6177-1.2495a.077.077 0 00-.0785-.037 19.7363 19.7363 0 00-4.8852 1.515.0699.0699 0 00-.0321.0277C.5334 9.0458-.319 13.5799.0992 18.0578a.0824.0824 0 00.0312.0561c2.0528 1.5076 4.0413 2.4228 5.9929 3.0294a.0777.0777 0 00.0842-.0276c.4616-.6304.8731-1.2952 1.226-1.9942a.076.076 0 00-.0416-.1057c-.6528-.2476-1.2743-.5495-1.8722-.8923a.077.077 0 01-.0076-.1277c.1258-.0943.2517-.1923.3718-.2914a.0743.0743 0 01.0776-.0105c3.9278 1.7933 8.18 1.7933 12.0614 0a.0739.0739 0 01.0785.0095c.1202.099.246.1981.3728.2924a.077.077 0 01-.0066.1276 12.2986 12.2986 0 01-1.873.8914.0766.0766 0 00-.0407.1067c.3604.698.7719 1.3628 1.225 1.9932a.076.076 0 00.0842.0286c1.961-.6067 3.9495-1.5219 6.0023-3.0294a.077.077 0 00.0313-.0552c.5004-5.177-.8382-9.6739-3.5485-13.6604a.061.061 0 00-.0312-.0286zM8.02 15.3312c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9555-2.4189 2.157-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.9555 2.4189-2.1569 2.4189zm7.9748 0c-1.1825 0-2.1569-1.0857-2.1569-2.419 0-1.3332.9554-2.4189 2.1569-2.4189 1.2108 0 2.1757 1.0952 2.1568 2.419 0 1.3332-.946 2.4189-2.1568 2.4189Z"/></svg>,
    x: <svg fill="currentColor" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>X</title><path d="M14.234 10.162 22.977 0h-2.072l-7.591 8.824L7.251 0H.258l9.168 13.343L.258 24H2.33l8.016-9.318L16.749 24h6.993zm-2.837 3.299-.929-1.329L3.076 1.56h3.182l5.965 8.532.929 1.329 7.754 11.09h-3.182z"/></svg>,
    facebook: <svg fill="currentColor" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>Facebook</title><path d="M9.101 23.691v-7.98H6.627v-3.667h2.474v-1.58c0-4.085 1.848-5.978 5.858-5.978.401 0 .955.042 1.468.103a8.68 8.68 0 0 1 1.141.195v3.325a8.623 8.623 0 0 0-.653-.036 26.805 26.805 0 0 0-.733-.009c-.707 0-1.259.096-1.675.309a1.686 1.686 0 0 0-.679.622c-.258.42-.374.995-.374 1.752v1.297h3.919l-.386 2.103-.287 1.564h-3.246v8.245C19.396 23.238 24 18.179 24 12.044c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.628 3.874 10.35 9.101 11.647Z"/></svg>,
    linkedin: <svg fill="currentColor" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>LinkedIn</title><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>,
    telegram: <svg fill="currentColor" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>Telegram</title><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>,
    email: <svg fill="currentColor" role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><title>Email</title><path d="M24 5.457v13.909c0 .904-.732 1.636-1.636 1.636h-3.819V11.73L12 16.64l-6.545-4.91v9.273H1.636A1.636 1.636 0 0 1 0 19.366V5.457c0-2.023 2.309-3.178 3.927-1.964L5.455 4.64 12 9.548l6.545-4.91 1.528-1.145C21.69 2.28 24 3.434 24 5.457z"/></svg>,
  };
  return <span className={`brand-logo ${brand}`} aria-hidden="true">{svgs[brand]}</span>;
}
  ...
  useEffect(() => {
    ...
    translateWindow.googleTranslateElementInit = () => {
      ...
      if (validLanguage !== pageLanguage) {
        window.setTimeout(() => applyLanguageWhenReady(validLanguage), 50);
      }
    };
...
function FireCrystalsCalculator() {
  const [fromLevel, setFromLevel] = useState("30");
  const [toLevel, setToLevel] = useState("FC10");
  const [dailyRfc, setDailyRfc] = useState(7);
  const [stock, setStock] = useState<FireCrystalStock>(fireCrystalEmptyStock);
  const calculation = useMemo(() => fireCrystalCostFor(fromLevel, toLevel), [fromLevel, toLevel]);
  const shortfall = useMemo(
    () =>
      fireCrystalStockFields.map((field) => ({
        ...field,
        needed: calculation.total[field.key],
        owned: stock[field.key],
        missing: Math.max(0, calculation.total[field.key] - stock[field.key]),
      })),
    [calculation.total, stock],
  );
  const firstGate = calculation.steps.find((step) => step.prerequisite !== "-");
  const rfcDays = calculation.total.refinedFireCrystals > 0 ? Math.ceil(Math.max(0, calculation.total.refinedFireCrystals - stock.refinedFireCrystals) / Math.max(1, dailyRfc)) : 0;
  const readyResources = shortfall.every((item) => item.missing === 0);

  const updateStock = (key: keyof FireCrystalStock, value: string) => {
    setStock((current) => ({
      ...current,
  ...
  return (
    <section className="home-page fire-crystal-page" id="fire-crystals" aria-label="Whiteout Survival Fire Crystal calculator">
      <section className="fire-crystal-hero">
        <div className="fire-crystal-hero-copy">
          <span className="section-kicker">Fire Crystal Calculator</span>
          <h1>Whiteout Survival Fire Crystal Calculator</h1>
          <p>Plan Furnace 30 to FC10 with sub-level costs, Refined Fire Crystal gates, resources, time, prerequisites, and current-stock shortfalls.</p>
          <div className="chief-charm-hero-chips fire-crystal-chips" aria-label="Fire Crystal calculator facts">
            <span><Icon name="flame" /> FC1-FC10 path</span>
            <span><Icon name="database" /> {formatNumber(fireCrystalCostFor("30", "FC10").total.fireCrystals)} FC total</span>
            <span><Icon name="shield" /> {formatNumber(fireCrystalCostFor("30", "FC10").total.refinedFireCrystals)} RFC total</span>
            <span><Icon name="calendar" /> Day 60/150/315/500 gates</span>
          </div>
        </div>
        <div className="fire-crystal-logo-stack" aria-label="Whiteout Survival branding">
          <img className="fire-crystal-main-logo" src="/whiteout-survival-logo.png" alt="Whiteout Survival logo" />
          <div className="fire-crystal-orbit">
            <img src="/svs-resources/fire-crystal.png" alt="Fire Crystal" />
            <strong>{readyResources ? "Ready" : "Need"}</strong>
            <span>{formatNumber(shortfall.find((item) => item.key === "fireCrystals")?.missing || 0)} FC</span>
          </div>
        </div>
      </section>

      <section className="fire-crystal-summary" aria-label="Fire Crystal totals">
        <article className="primary">
          <span>Fire Crystals</span>
          <strong>{formatNumber(calculation.total.fireCrystals)}</strong>
          <small>{formatNumber(shortfall[0].missing)} missing from stock</small>
        </article>
        <article>
          <span>Refined FC</span>
          <strong>{formatNumber(calculation.total.refinedFireCrystals)}</strong>
          <small>{rfcDays ? `${formatNumber(rfcDays)} lab day${rfcDays === 1 ? "" : "s"} at ${dailyRfc}/day` : "No RFC gate"}</small>
        </article>
        <article>
          <span>Base Time</span>
          <strong>{formatNumber(calculation.days)}d</strong>
          <small>Before construction buffs</small>
        </article>
        <article className={readyResources ? "ready" : "short"}>
          <span>Power Gain</span>
          <strong>{formatNumber(calculation.power)}</strong>
          <small>{calculation.steps.length} upgrade step{calculation.steps.length === 1 ? "" : "s"}</small>
        </article>
      </section>

      <section className="fire-crystal-workbench" aria-label="Fire Crystal planning controls">
        <article className="fire-crystal-panel">
          <div className="chief-charm-panel-head">
            <span>Plan</span>
            <strong>Choose the current completed Furnace level and target level</strong>
          </div>
          <div className="charm-form-grid fire-crystal-form">
            <label>
              ...
              <select
                ...
                onChange={(event) => {
                  ...
                  setFromLevel(next);
                  if (fireCrystalOptionIndex(toLevel) <= fireCrystalOptionIndex(next)) {
                    setToLevel(fireCrystalSteps[Math.min(fireCrystalSteps.length - 1, fireCrystalOptionIndex(next) + 1)]?.id || "FC10");
                  }
                }}
              >
                {fireCrystalStartOptions.slice(0, -1).map((option) => (
            ...
            <label>
              ...
              <select value={toLevel} onChange={(event) => setToLevel(event.currentTarget.value)}>
                {fireCrystalSteps.map((step) => (
                  <option value={step.id} disabled={fireCrystalOptionIndex(step.id) <= fireCrystalOptionIndex(fromLevel)} key={step.id}>
                    {step.label}
                  </option>
                ))}
              </select>
            ...
            <label>
              <span>RFC Lab / Day</span>
              <input value={dailyRfc} onChange={(event) => setDailyRfc(Number(event.currentTarget.value.replace(/\D/g, "")) || 1)} inputMode="numeric" />
            </label>
          </div>
          <div className="fire-crystal-gate">
            <Icon name="shield" />
            <span>
              <strong>{firstGate?.prerequisite || "No new support-building gate in this range"}</strong>
              <small>Furnace FC sub-levels are linear; support buildings gate the first sub-level of each tier.</small>
            </span>
          </div>
        </article>
        ...
        <article className="fire-crystal-panel">
          <div className="chief-charm-panel-head">
            <span>Inventory</span>
            <strong>Enter what you already have to see exact shortfalls</strong>
          </div>
          <div className="fire-crystal-stock-grid">
            {fireCrystalStockFields.map((field) => (
              <label key={field.key}>
                <span><img src={field.icon} alt="" /> {field.label}</span>
                <input value={stock[field.key] || ""} onChange={(event) => updateStock(field.key, event.currentTarget.value)} inputMode="decimal" placeholder="0" />
              </label>
            ))}
          </div>
        </article>
      ...
      <section className="fire-crystal-shortfall-grid" aria-label="Resource shortfall">
        {shortfall.map((item) => (
          <article className={item.missing ? "short" : "ready"} key={item.key}>
            <img src={item.icon} alt="" />
            <span>{item.label}</span>
            <strong>{formatNumber(item.missing)}{item.suffix || ""}</strong>
            <small>Need {formatNumber(item.needed)}{item.suffix || ""} · Own {formatNumber(item.owned)}{item.suffix || ""}</small>
          </article>
        ))}
      </section>

      <section className="fire-crystal-data-grid" aria-label="Fire Crystal reference">
        <article className="fire-crystal-panel">
          <div className="chief-charm-panel-head">
            <span>Tier Totals</span>
            <strong>Per-tier FC/RFC costs for the Furnace path</strong>
          </div>
          <div className="fire-crystal-table-wrap">
            <table className="fire-crystal-table">
              <thead>
                <tr><th>Tier</th><th>FC</th><th>RFC</th><th>Time</th><th>Gate</th></tr>
              </thead>
              <tbody>
                {fireCrystalTierSummary.map((tier) => (
                  <tr key={tier.tier}>
                    <td><strong>{tier.tier}</strong></td>
                    <td>{formatNumber(tier.total.fireCrystals)}</td>
                    <td>{formatNumber(tier.total.refinedFireCrystals)}</td>
                    <td>{formatNumber(tier.days)}d</td>
                    <td>{tier.prerequisite}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <article className="fire-crystal-panel">
          <div className="chief-charm-panel-head">
            ...
            <strong>{calculation.steps.length ? `${calculation.steps[0].label} to ${calculation.steps[calculation.steps.length - 1].label}` : "No upgrade selected"}</strong>
          </div>
          <div className="fire-crystal-step-list">
            {calculation.steps.slice(0, 12).map((step) => (
              <div className="fire-crystal-step" key={step.id}>
                <span>{step.label}</span>
                <strong>{formatNumber(step.fireCrystals)} FC{step.refinedFireCrystals ? ` + ${formatNumber(step.refinedFireCrystals)} RFC` : ""}</strong>
                <small>{step.days}d · {step.prerequisite}</small>
              </div>
            ))}
            {calculation.steps.length > 12 && <div className="fire-crystal-step more">+{calculation.steps.length - 12} more steps included in totals</div>}
          </div>
        </article>
      </section>

      <section className="fire-crystal-sources" aria-label="Fire Crystal data sources">
        <a href="https://www.woscalculator.com/fire-crystals" target="_blank" rel="noreferrer">WOSCalculator <Icon name="external" /></a>
        <a href="https://wos.h5joy-games.com/buildings/furnace/" target="_blank" rel="noreferrer">WOS Guide/H5Joy <Icon name="external" /></a>
...
export function HomeApp({ initialMenu = "home" }: { initialMenu?: ActiveMenu } = {}) {
  ...
  const [sneakPeekData, setSneakPeekData] = useState<SneakPeekData | null>(null);
  const [loadingSneakPeek, setLoadingSneakPeek] = useState(false);
  ...
  }, [activeMenu, sneakPeekData, loadingSneakPeek]);
  ...
  const wikiMenuActive = activeMenu === "wikiHeroes" || activeMenu === "wikiBuildings" || activeMenu === "wikiPosters";
  const calculatorMenuActive = activeMenu === "chiefGear" || activeMenu === "chiefCharm" || activeMenu === "vip" || activeMenu === "warAcademy";
  ...
  const posterCategoryCounts = scrapedWosPosters.reduce<Record<string, number>>((counts, poster) => {
    counts[poster.category] = (counts[poster.category] || 0) + 1;
    return counts;
  }, {});
  ...
  const filteredWosPosters = activePosterCategory === "All"
    ? scrapedWosPosters
    : scrapedWosPosters.filter((poster) => poster.category === activePosterCategory);
  ...
  const trimCanvasText = (context: CanvasRenderingContext2D, text: string, maxWidth: number) => {
  ...
  const drawFoundryAvatar = (
    ...
    const radius = size / 2;
    context.fillStyle = avatar ? "#f8fafc" : color;
    context.beginPath();
    ...
    if (avatar) {
      ...
      context.clip();
      context.drawImage(avatar, centerX - radius + 1, centerY - radius + 1, size - 2, size - 2);
      context.restore();
    } else {
      context.fillStyle = textColor;
      context.font = `900 ${Math.max(11, Math.round(size * 0.38))}px Arial`;
      context.textAlign = "center";
      context.textBaseline = "middle";
      context.fillText(label.slice(0, 1).toUpperCase(), centerX, centerY + 1);
  ...
  const drawFoundryMemberChip = (
    ...
    drawFoundryAvatar(context, avatar, x + 28, y + 24, 32, color, label);
    ...
    context.fillText(trimCanvasText(context, label, width - 62), x + 52, y + 30);
    if (member.profile) {
      context.fillStyle = "#cbd5e1";
      context.font = "800 9px Arial";
      context.fillText(trimCanvasText(context, `Furnace ${furnaceDisplay(member.profile)}`, width - 62), x + 52, y + 42);
    }
  };
  ...
  const loadFoundryAvatarMapForExport = async () => {
    const members = allFoundryTeams.flatMap((team) => [team.rallyLeader, ...team.joiners]);
    ...
    return new Map<string, HTMLImageElement | null>(avatarEntries);
  };
  ...
  const exportFoundryMapImage = async () => {
    ...
    try {
      ...
      const avatarImages = await loadFoundryAvatarMapForExport();
  ...
  const exportFoundryTableImage = async () => {
    ...
    canvas.height = 150 + rowHeights.reduce((total, height) => total + height + cardGap, 0) + Math.max(0, cardRows - 1) * cardGap + cardPadding;
    const context = canvas.getContext("2d");
    if (!context) {
    ...
    teamCards.forEach((card, index) => {
      ...
      card.members.forEach((member, memberIndex) => {
        ...
        drawFoundryAvatar(context, avatarImages.get(member.id), x + 132, rowY - 7, 24, member.role === "leader" ? color : "#64748b", foundryMemberName(member));
        context.fillStyle = "#e5e7eb";
        context.font = "800 13px Arial";
        context.fillText(member.playerId || "-", x + 172, rowY - 2);
        context.fillText(trimCanvasText(context, foundryMemberName(member), 126), x + 286, rowY - 2);
  ...
  const exportFoundryCombinedImage = async () => {
    setFoundryExportStatus("Preparing compact battle sheet...");
    try {
      const [mapImage, logo, avatarImages] = await Promise.all([
        loadFoundryMapForExport(),
        loadFoundryLogoForExport(),
        loadFoundryAvatarMapForExport(),
      ]);
      const teamCards = allFoundryTeams.map((team) => ({
        team,
        building: foundryBuildings.find((item) => item.id === team.buildingId),
      ...
      const tableHeight = teamCards.reduce((total, card) => {
        ...
        return total + 72 + rows.length * rowHeight + cardGap;
      }, 0);
      ...
      foundryBuildings.forEach((building) => {
        ...
        const y = mapY + (building.y / 100) * mapHeight;
        const labelWidth = building.name.length > 16 ? 190 : 132;
        context.fillStyle = "rgba(12, 18, 20, 0.82)";
        context.strokeStyle = building.phase === "Spawn" ? "#38bdf8" : building.id.includes("workshop") ? "#22c55e" : "#fbbf24";
        context.lineWidth = 2;
      ...
      allFoundryTeams.forEach((team, teamIndex) => {
        const building = foundryBuildings.find((item) => item.id === team.buildingId) || foundryBuildings[2];
        const teamColor = foundryTeamColors[teamIndex % foundryTeamColors.length];
        const centerX = mapX + (building.x / 100) * mapWidth;
        const centerY = mapY + (building.y / 100) * mapHeight;
        const members = [team.rallyLeader, ...team.joiners].filter((member) => member.playerId || member.profile);
        ...
        const buildingTeamIndex = teamsAtBuilding.findIndex((item) => item.id === team.id);
        const teamOffset = Math.max(0, buildingTeamIndex);
        const labelY = Math.max(mapY + 46, centerY - 140 - teamOffset * 22);
        ...
        context.lineWidth = 2;
        context.beginPath();
        context.roundRect(centerX - 140, labelY - 22, 280, 44, 14);
        context.fill();
        context.stroke();
        context.restore();
        context.fillStyle = "rgba(255, 255, 255, 0.1)";
        context.beginPath();
        context.roundRect(centerX - 134, labelY - 16, 268, 12, 8);
        context.fill();
        context.textAlign = "center";
        context.fillStyle = teamColor;
        context.font = "900 13px Arial";
        ...
        context.setLineDash([8, 10]);
        context.beginPath();
        context.ellipse(centerX, centerY, 160 + teamOffset * 22, 96 + teamOffset * 14, 0, 0, Math.PI * 2);
        context.stroke();
        context.setLineDash([]);

        members.forEach((member, memberIndex) => {
          const offset = foundryExportCircleOffset(memberIndex, members.length, teamOffset);
          const chipWidth = member.role === "leader" ? 164 : 150;
          const rawX = centerX + offset.x * (mapWidth / 1800);
          const rawY = centerY + offset.y * (mapHeight / 1452);
          const x = Math.max(mapX + 8, Math.min(mapX + mapWidth - chipWidth - 8, rawX - chipWidth / 2));
          const y = Math.max(mapY + 54, Math.min(mapY + mapHeight - 64, rawY - 24));
          drawFoundryMemberChip(context, member, x, y, chipWidth, member.role === "leader" ? foundryLeaderColor : teamColor, avatarImages.get(member.id));
        });
      });
      ...
      context.font = "800 14px Arial";
      context.textAlign = "right";
      context.fillText("WhiteoutSurvival.dev", canvas.width - 42, canvas.height - 28);

      downloadCanvas(canvas, `foundry-legion-${foundryLegion}-battle-sheet.png`);
      setFoundryExportStatus("Compact battle sheet downloaded.");
    } catch (error) {
      setFoundryExportStatus(error instanceof Error ? error.message : "Unable to export compact battle sheet.");
    }
  };
  ...
  const exportFoundryPlanImages = async () => {
    ...
    await exportFoundryCombinedImage();
  };

  const navigateToMenu = (menu: ActiveMenu) => {
    setMobileMoreOpen(false);
    if (menu !== "wikiHeroes" && menu !== "wikiBuildings" && menu !== "wikiPosters") {
      setSidebarWikiOpen(false);
    }
    if (menu !== "chiefGear" && menu !== "chiefCharm" && menu !== "vip" && menu !== "warAcademy") {
      setSidebarCalculatorOpen(false);
    }
    setActiveMenu(menu);
    setActiveWikiSlug("");
    ...
    >
      ...
      <div className="app-body">
        ...
        <div className="content-column">
          ...
          {activeMenu === "home" ? (
              ...
              {[
                {
                  ...
                  items: [
                    ...
                    ["warAcademy" as const, "book", "War Academy", "Plan T11 Helios research, shard shortfalls, and research time."],
          ...
          ) : activeMenu === "fireCrystals" ? (
            <FireCrystalsCalculator />
          ) : activeMenu === "warAcademy" ? (
            <WarAcademyCalculator />
          ) : activeMenu === "vip" ? (
            ...
            <section className="home-page wiki-page" id="wiki-heroes" aria-label="Whiteout Survival wiki heroes">
              <section className="wiki-hero">
                ...
                <div className="wiki-switcher" aria-label="Wiki section switcher">
                  ...
                  <button type="button" onClick={() => navigateToMenu("wikiPosters")}>Posters</button>
                </div>
              </section>
          ...
          ) : activeMenu === "wikiPosters" ? (
            <section className="home-page wiki-page wiki-posters-page" id="wiki-posters" aria-label="Whiteout Survival official posters">
              <section className="wiki-hero wiki-posters-hero">
                <div>
                  <span className="section-kicker">Whiteout Survival Gallery</span>
                  <h1>Official Posters Gallery</h1>
                  <p>Browse locally mirrored Whiteout Survival poster art by heroes, events, festivals, and survivors.</p>
                </div>
                <div className="wiki-switcher" aria-label="Wiki section switcher">
                  <button type="button" onClick={() => navigateToMenu("wikiHeroes")}>Heroes</button>
                  <button type="button" onClick={() => navigateToMenu("wikiBuildings")}>Buildings</button>
                  <button className="active" type="button" onClick={() => navigateToMenu("wikiPosters")}>Posters</button>
                </div>
              </section>

              <section className="wiki-summary wiki-posters-summary" aria-label="Poster gallery summary">
                <article>
                  <span>Total Posters</span>
                  <strong>{scrapedWosPosters.length}</strong>
                </article>
                {posterCategories.slice(0, 3).map((category) => (
                  <article key={category}>
                    <span>{category}</span>
                    <strong>{posterCategoryCounts[category] || 0}</strong>
                  </article>
                ))}
              </section>

              <section className="wiki-panel" aria-label="Poster list">
                <div className="wiki-panel-head">
                  <div>
                    <h2>{activePosterCategory === "All" ? "All Posters" : `${activePosterCategory} Posters`}</h2>
                    <p>{filteredWosPosters.length} local poster image{filteredWosPosters.length === 1 ? "" : "s"} available in this gallery.</p>
                  </div>
                </div>
                <div className="wiki-building-tabs wiki-poster-tabs" aria-label="Poster filters">
                  {["All", ...posterCategories].map((category) => (
                    <button
                      className={activePosterCategory === category ? "active" : ""}
                      type="button"
                      key={category}
                      onClick={() => setActivePosterCategory(category)}
                    >
                      <Icon name={category === "All" ? "grid" : category === "Heroes" ? "user" : category === "Events" ? "star" : category === "Festivals" ? "gift" : "image"} />
                      <span>{category}</span>
                      <small>{category === "All" ? scrapedWosPosters.length : posterCategoryCounts[category] || 0}</small>
                    </button>
                  ))}
                </div>
            ...
            <section className="home-page sneak-peek-page" id="sneak-peek" aria-label="Chief Concierge Sneak Peek">
              <section className="sneak-peek-hero">
                ...
                <p>Latest event details, rewards, skins, packs, and gift code information from the official sneak peek.</p>
              </section>

              <article className="sneak-peek-frame">
                <header className="sneak-peek-frame-title">
                  <h2>{sneakPeekData ? sneakPeekData.title : "Chief Concierge: Sneak Peek"}</h2>
          ...
          <section className="bottom-video-section" style={{ width: "100%", maxWidth: "1080px", margin: "60px auto 40px", padding: "0 22px" }} aria-label="Official Whiteout Survival Video">
            <div style={{ position: "relative", width: "100%", borderRadius: "12px", overflow: "hidden", boxShadow: "0 14px 34px rgba(0, 0, 0, 0.2)", border: "1px solid var(--border)", background: "var(--card)", display: "flex" }}>
              <video 
                src="/official-background.mp4" 
                autoPlay 
                muted 
                loop 
                playsInline 
          ...
          <section className="bottom-community-section" aria-label="Whiteout Survival resource links">
            ...
            <nav className="bottom-community-links" aria-label="End of page links">
              <div>
                ...
                <a href="/war-academy-calculator" onClick={(event) => { event.preventDefault(); navigateToMenu("warAcademy"); }}>War Academy Calculator</a>
